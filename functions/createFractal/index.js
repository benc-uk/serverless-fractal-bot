...
module.exports = async function (context, req) {
  ...
  const type =        req.query.type || 'mandelbrot'
  ...
  const hue =         parseInt(req.query.hue || 120)
  const hueLoops =    parseFloat(req.query.hueLoops || 1)
  const sat =         parseInt(req.query.sat || 100)
  const brightness =  parseInt(req.query.bright|| 150)
  const juliaR =      parseFloat(req.query.juliar || 0.362)
  const juliaI =      parseFloat(req.query.juliai || 0.370)
  const innerBright = parseFloat(req.query.innerBright || 0)
  ...
  for (var x = 0; x < bitmap.width; x++) {
    for (var y = 0; y < bitmap.height; y++) {
      ...
      if (iterations == maxiters) {
        ...
        const nIter = iterations / (maxiters - 1)
        // Luminance based on brightness factor
        const lumVal = nIter * brightness
        // Hue is calculated with a cosine to create repeating colour loops
        const hueVal = Math.cos(nIter * hueLoops) * hue
